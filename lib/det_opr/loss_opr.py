...
def softmax_loss(score, label, ignore_label=-1):
    ...
    max_score = score.max(axis=1, keepdims=True)[0].detach()
    score -= max_score
    log_prob = score - torch.log(torch.exp(score).sum(axis=1, keepdims=True))
    log_prob = -log_prob[~label.eq(ignore_label)]
    vlabel = label[~label.eq(ignore_label)]
    loss = log_prob[torch.arange(log_prob.shape[0]), vlabel.long()].mean()
    return loss
