...
class RCNN(nn.Module):
    ...
    def forward(self, fpn_fms, rcnn_rois, labels=None, bbox_targets=None):
        ...
        if self.training:
            ...
            loss_emd = loss_emd.sum()/loss_emd.shape[0]
            ...
            pred_scores_0 = F.softmax(pred_emd_pred_cls_0, dim=-1)
            pred_scores_1 = F.softmax(pred_emd_pred_cls_1, dim=-1)
            ...
            pred_bbox_0 = torch.cat([pred_bbox_0, pred_scores_0[:, 1].reshape(-1,1)], dim=1)
            pred_bbox_1 = torch.cat([pred_bbox_1, pred_scores_1[:, 1].reshape(-1,1)], dim=1)
